...
<div class="card">
	<div class="card-header">
		<h5 class="mb-0"><i class="bi bi-clipboard me-2 text-primary"></i>Add Enrollment</h5>
	</div>
	<div class="card-body">
				...
				<div class="col-md-6">
					...
					<select name="student_id" class="form-select" required>
						...
						@foreach ($students as $student)
							<option value="{{ $student->id }}" @selected(old('student_id') == $student->id)>{{ $student->student_id }} - {{ $student->full_name }}</option>
						@endforeach
					</select>
				...
				<div class="col-md-6">
					...
					<select name="course_id" class="form-select" required>
						...
						@foreach ($courses as $course)
							<option value="{{ $course->id }}" @selected(old('course_id') == $course->id)>{{ $course->course_code }} - {{ $course->course_title }}</option>
						@endforeach
					</select>
